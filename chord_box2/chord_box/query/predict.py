...
def _predict(r: int, n: list[int]):
    ...
    result = all(f'predict(chord({r}, {n}), K, GN, NewTmplR, NewTmplN, NewG)')
...
def _reverse(r: int, n: list[int]):
    r = r % 12
    n = sorted(set(i % 12 for i in n))
    result = all(f'reverse(chord({r}, {n}), K, GN, NewTmplR, NewTmplN, NewG)')
    x = integrate(result, 'K')
    x = {k: integrate(bytes_to_str(v, 'GN', 'NewG'), 'GN') for k, v in x.items()}
    return x
...
def _connect(r1: int, n1: list[int], r2: int, n2: list[int]):
    ...
    result = all(f'connect(chord({r1}, {n1}), chord({r2}, {n2}), G1, G2, K1, K2, NewG1, NewG2, NewTmplR, NewTmplN)')
